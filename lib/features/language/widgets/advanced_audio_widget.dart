...
class AdvancedAudioWidget extends ConsumerStatefulWidget {
  final QuizQuestion question;
  final Function(String) onAnswerSelected;
  final bool showResult;
  final bool isCorrect;

  const AdvancedAudioWidget({
    super.key,
  ...
  @override
  ConsumerState<AdvancedAudioWidget> createState() => _AdvancedAudioWidgetState();
}

class _AdvancedAudioWidgetState extends ConsumerState<AdvancedAudioWidget>
  ...
  Widget _buildAudioInterface(BuildContext context) {
    switch (widget.question.type) {
      case QuestionType.listening:
        return _buildListeningInterface(context);
      case QuestionType.speaking:
        return _buildSpeakingInterface(context);
      case QuestionType.pronunciation:
        return _buildPronunciationInterface(context);
      default:
        return _buildDefaultInterface(context);
    }
  }

  Widget _buildListeningInterface(BuildContext context) {
    return Column(
      children: [
        // Audio Waveform
        _buildAudioWaveform(context),
  ...
  Widget _buildSpeakingInterface(BuildContext context) {
    return Column(
      children: [
        // Target Text
        Container(
  ...
  Widget _buildPronunciationInterface(BuildContext context) {
    return Column(
      children: [
        // Word Display
        Container(
  ...
  Widget _buildDefaultInterface(BuildContext context) {
    return Column(
      children: [
        Text(
          widget.question.question,
  ...
  Widget _buildAudioWaveform(BuildContext context) {
    return Container(
      width: double.infinity,
      height: 120,
      decoration: BoxDecoration(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 12),
          if (widget.question.type == QuestionType.speaking)
            ElevatedButton(
  ...
  void _playAudio() async {
    ...
    final audioService = ref.read(ttsServiceProvider);
    await audioService.initialize();
    
    if (widget.question.type == QuestionType.listening) {
      await audioService.speakAmharic(widget.question.correctAnswer);
    } else if (widget.question.type == QuestionType.pronunciation) {
      await audioService.speakAmharic(widget.question.correctAnswer);
    }
